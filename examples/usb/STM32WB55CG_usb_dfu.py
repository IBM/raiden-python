...
raiden = raiden.Raiden(mhz=100, serial_dev="/dev/ttyUSB1", baud= 115200, ticks= True, debug=False)
start_time = time.time()
cs = ChipSHOUTER("/dev/ttyUSB2")
cs.voltage = 350
cs.reset = 0
cs.reset = 1
cs.armed = 1
        ...
        while width <= width_range.max:
            while repeat <= repeat_range.max:
                ...
                try:
                    ...
                    continue
                try:
                    resp = dev.ctrl_transfer(bmRequestType=0x81, bRequest=0x6, wValue=0x2100, wIndex=0x0000, data_or_wLength=65000)
                except Exception as e:
